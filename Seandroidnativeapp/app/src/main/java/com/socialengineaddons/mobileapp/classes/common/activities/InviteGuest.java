...
public class InviteGuest extends AppCompatActivity implements TextWatcher, AdapterView.OnItemClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mToolBarTitle = getIntent().getExtras().getString(ConstantVariables.KEY_TOOLBAR_TITLE);
        mSnackBarMessage = getIntent().getExtras().getString(ConstantVariables.KEY_SUCCESS_MESSAGE);
        SCHEMA_KEY_SEND_INVITES = getIntent().getExtras().getString(ConstantVariables.SCHEMA_KEY_SEND_INVITES);
            ...
            public void onTaskCompleted(JSONObject body) {
                if (body != null && body.length() != 0) {
                    ...
                    for (int i = 0; i < guestListResponse.length(); i++) {
                        ...
                        String userImage = friendObject.optString("image_profile");
    ...
    public void sendInviteRequest() {
        ...
        if (!isGuestListBlank) {
            ...
            if (mCurrentSelectedModule != null
                ...
                SCHEMA_KEY_SEND_INVITES = "friends_id";
            } else if (!(SCHEMA_KEY_SEND_INVITES != null && !SCHEMA_KEY_SEND_INVITES.isEmpty())) {
                SCHEMA_KEY_SEND_INVITES = "user_ids";
            }
            postParams.put(SCHEMA_KEY_SEND_INVITES, res);

            if (mCurrentSelectedModule.equals(ConstantVariables.ADV_VIDEO_CHANNEL_MENU_TITLE)) {
                mAppConst.getJsonResponseFromUrl(inviteFormUrl, new OnResponseListener() {
                    @Override
