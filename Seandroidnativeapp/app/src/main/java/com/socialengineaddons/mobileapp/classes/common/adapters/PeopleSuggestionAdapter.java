...
public class PeopleSuggestionAdapter extends RecyclerView.
    ...
    private boolean mIsFeedSuggestion = false;
    ...
    public PeopleSuggestionAdapter(Context context, List<Object> browseList, String suggestionView,
                                   boolean isFeedSuggestion, OnItemDeleteListener onItemDeleteListener,
                                   OnAddRequestSuccessListener onAddRequestSuccess) {
        mContext = context;
        mBrowseList = browseList;
        mSuggestionViewType = suggestionView;
        mOnItemDeleteListener = onItemDeleteListener;
        mIsFeedSuggestion = isFeedSuggestion;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        switch (viewType) {
            ...
            case SUGGESTION_TYPE:
                ...
                mImageLoader.setImageForPeopleSuggestion(userProfileImage, holder.ivUserProfile);

                // Showing member name.
                holder.tvUserName.setText(mBrowseListItems.getDisplayName());

                /* Get Add Friends Label form Menus Array
                 * because it can be diff in case of one way friendship
                 */
                String addFriendsLabel = mContext.getResources().getString(R.string.add_friend_title);
                if (mBrowseListItems.getMenuArray() != null) {
                    for (int i = 0; i < mBrowseListItems.getMenuArray().length(); i++) {
                        JSONObject menuObj = mBrowseListItems.getMenuArray().optJSONObject(i);
                        if (menuObj.optString("name").equals("add_friend")) {
                            addFriendsLabel = menuObj.optString("label");
                            break;
                        }
                    }
                }

                if (mIsFeedSuggestion) {

                    // Getting drawable for add suggestion.
                    Drawable addDrawable = ContextCompat.getDrawable(mContext, R.drawable.ic_person_add_white_24dp);
                    addDrawable.setColorFilter(new PorterDuffColorFilter(ContextCompat.getColor(mContext, R.color.themeButtonColor),
                            PorterDuff.Mode.SRC_ATOP));
                    holder.tvAddFriend.setCompoundDrawablesWithIntrinsicBounds(addDrawable, null, null, null);

                    if (mSuggestionViewType.equals("list_view")) {
                        holder.tvCancelRequest.setText(mContext.getResources().
                                getString(R.string.cancel_dialogue_message));
                    } else {
                        holder.tvCancelRequest.setText(mContext.getResources().
                                getString(R.string.group_cancel_request_dialogue_button));
                    }

                    holder.tvAddFriend.setText(addFriendsLabel);
                    holder.tvRemoveSuggestion.setText(mContext.getResources().getString(R.string.remove_listing_dialogue_button));
                    ...
                    holder.tvAddFriend.setText(addFriendsLabel.toUpperCase());
