...
public class FormMultiOptions extends FormWidget implements View.OnClickListener {
    ...
    private LinearLayout llSubscriptionDetails;
    ...
    private void initializeConstructorValues(String label, String description) {
            ...
            while (keys.hasNext()) {
                ...
                switch (mFieldName) {
                    ...
                    case "main_channel_id":
                        ...
                        break;
                    default:
                        if (joProperty.optString("layoutType", "1").equals("2")) {
                            mOptionsItemList.add(new SheetItemModel(jsonObjectOptions.optJSONObject(key), key, "1"));
                        } else {
                            mOptionsItemList.add(new SheetItemModel(getLabelFromKey(key), key));
                        }
                        break;
        ...
        if (ConstantVariables.FORM_LAYOUT_TYPE == 1) {
             configFieldView = mLayoutInflater.inflate(R.layout.layout_form_select_option_1, null);
        } else {
             configFieldView = mLayoutInflater.inflate(R.layout.layout_form_select_option, null);
        }
        ...
        mSheetAdapter.setOnItemClickListener(new SimpleSheetAdapter.OnItemClickListener() {
            ...
            public void onItemClick(SheetItemModel item, int position) {
                ...
                if (item.getKey().equals("create_channel")) {
                    ...
                    /* Subscription info display work in signup form */
                    if (mCurrentSelectedOption.equals(ConstantVariables.SUBSCRIPTION_ACCOUNT) && joProperty.optString("name").equals("package_id")) {
                        checkVisibility(item.getKey());
                    }
                    ...
                    if (!mFieldValue.equals(item.getKey())) {
                        ...
                        if (!isCreateForm && mCurrentSelectedOption.equals(ConstantVariables.EDIT_PRODUCT)
                                && mFieldName.equals("category_id")) {
                            String key = String.valueOf(FormActivity.getFormObject().optJSONObject("formValues").optInt("category_id"));

                            if (key.equals(mFieldValue)) {
                                JSONObject formObject = FormActivity.getFormObject().optJSONObject("fields");
                                removeEditFiled(mFieldName, mFieldValue , formObject);
                            }
                        }
    ...
    private void getViews(View configFieldView, String label, String description) {
        ...
        if (mCurrentSelectedOption.equals(ConstantVariables.SUBSCRIPTION_ACCOUNT) && joProperty.optString("name").equals("package_id")) {
            llSubscriptionDetails = configFieldView.findViewById(R.id.subscription_details_view);;
        }
        ...
        if (joProperty.has("value")) {
            setValue(joProperty.optString("value"));
        }
...
//        }
    ...
    private void checkVisibility() {
        ...
        if ((joProperty.optString(FormActivity.SCHEMA_KEY_TYPE).equals(FormActivity.SCHEMA_KEY_RADIO) || joProperty.optString(FormActivity.SCHEMA_KEY_TYPE).equals(FormActivity.SCHEMA_KEY_RADIO_SMALLER))
     ...
     * Method to checkVisibility of the other views which are need to shown on a particular item click.
     */
    private void checkVisibility(String key) {

        JSONObject selectedObj = jsonObjectOptions.optJSONObject(key);

        if (selectedObj != null && selectedObj.has("type") && selectedObj.optString("type").equals("trial")) {
            JSONArray jsonArray = selectedObj.optJSONArray("trialfields");
            if (jsonArray.length() > 0) {
                llSubscriptionDetails.removeAllViews();
                for (int i = 0; i < jsonArray.length(); i++) {
                    createSubscriptionFieldView(jsonArray.optString(i));
                }
    ...
    private void createSubscriptionFieldView(String  label) {
        ...
        textView.setTextSize(TypedValue.COMPLEX_UNIT_PX, mContext.getResources().getDimension(R.dimen.body_medium_font_size));
        layoutParams.setMargins(0, (int) mContext.getResources().getDimension(R.dimen.margin_10dp), 0, 0);
        textView.setLayoutParams(layoutParams);
        textView.setGravity(Gravity.CENTER);


        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            textView.setText(Html.fromHtml(label, Html.FROM_HTML_MODE_LEGACY));
        }
        llSubscriptionDetails.addView(textView);
    ...
    private String getLabelFromKey(String key) {
        ...
        if (subscriptionDetails != null && subscriptionDetails.length() > 0 && subscriptionDetails.has("label")) {
            ...
            if (price != 0) {
                String priceWithCurrency = CurrencyUtils.getFormattedCurrencyString(
    ...
    public void setValue(String value) {
        ...
        if (value != null && etFieldValue != null && jsonObjectOptions != null
            ...
            String label = getLabelFromKey(value);

            etFieldValue.setText(label);
            etFieldValue.setTag(value);
            mFieldValue = value;
            checkVisibility();
        }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.form_main_view:
                AppConstant mAppConstant = new AppConstant(mContext);
                mAppConstant.hideKeyboard();
    ...
    private void removeChild(String parentName){
        ...
        String append, child;
        JSONObject formObject, multiChild;

        if (mCurrentSelectedOption.equals(ConstantVariables.SIGN_UP_FIELDS)) {
            append = FormActivity.getAttribByPropertyForSignupFields(parentName,"append",null);
            formObject = FormActivity.getSignupProfileFields();
            child = FormActivity.getRegistryByPropertyForSignupFields(parentName,"child");
            multiChild = FormActivity.getRegistryByPropertyForSignupFields(parentName, "multiChild", "multicheckbox");

        } else {
            append = FormActivity.getAttribByProperty(parentName,"append",null);
            formObject = FormActivity.getFormObject().optJSONObject("fields");
            child = FormActivity.getRegistryByProperty(parentName,"child");
            multiChild = FormActivity.getRegistryByProperty(parentName, "multiChild", "multicheckbox");
    ...
    private void inflateSubFormView(String key) {
        ...
        JSONArray subFormArray = formObject.optJSONArray(keyName);

        if(subFormArray != null) {
            String append;
            if (mCurrentSelectedOption.equals(ConstantVariables.SIGN_UP_FIELDS)){
                append = FormActivity.getAttribByPropertyForSignupFields(mFieldName,"uppend",null);
            } else {
                append = FormActivity.getAttribByProperty(mFieldName,"uppend",null);
            }
            ...
            for (int i = 0; i < subFormArray.length(); ++i) {
                ...
                if (fieldsObject != null) {
                    ...
                    mFormWidget = mFormActivity.getWidget(mContext, name, fieldsObject, label, hasValidator, false, null, mFormWidgetList,
        ...
        if (mCurrentSelectedOption.equals(ConstantVariables.SIGN_UP_FIELDS)) {
            FormActivity.setRegistryByPropertyForSingupFields(mFieldName, FormActivity.getSignupProfileFields().optJSONObject(mFieldName), (!key.equals("")) ? mFieldName+key : null, "multiOptions", 0);
        } else {
            FormActivity.setRegistryByProperty(mFieldName, FormActivity.getFormObject().optJSONObject(mFieldName), (!key.equals("")) ? mFieldName+"_"+key : null, "multiOptions", 0);
        }
    }



    /* Remove edit form profile fields rendered with form on category change */
    private void removeEditFiled(String mFieldName, String mFieldValue, JSONObject formObject) {

        JSONArray subFormArray = formObject.optJSONArray(mFieldName + "_" + mFieldValue);
        for (int i = 0; i < subFormArray.length(); ++i) {
            JSONObject fieldsObject = subFormArray.optJSONObject(i);

            if (fieldsObject != null) {
                String name = fieldsObject.optString("name");
                FormWidget formWidget =  mFormWidgetMap.get(name);
                mFormWidgetList.remove(formWidget);
            }

            if (mFormWidgetMap.containsKey("subcategory_id")) {
                FormWidget formWidget =  mFormWidgetMap.get("subcategory_id");
                mFormWidgetList.remove(formWidget);
            }

            if (mFormWidgetMap.containsKey("subsubcategory_id")) {
                FormWidget formWidget =  mFormWidgetMap.get("subsubcategory_id");
                mFormWidgetList.remove(formWidget);
            }
        }
