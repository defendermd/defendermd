...
public class InfoTabFragment extends BaseFragment implements View.OnClickListener {
    ...
    private WebView mViewDescription, mOverviewContent, mViewBody;
    ...
    private LinearLayout mOwnerDetailBlock, mSimpleViewLayout, mWishListBlock, mOverviewView;
    ...
    public void initializeView(int mMLTViewType) {
        ...
        mOverviewView = (LinearLayout) mRootView.findViewById(R.id.overview_view);
        mOverviewContent = (WebView) mRootView.findViewById(R.id.overview_content);
    ...
    public void setDataInViews(int mMLTViewType, JSONObject mResponseJsonObject) {
        ...
        try {
            ...
            String overview = mResponseJsonObject.optString("overview");
            if (mMLTViewType == ConstantVariables.BLOG_VIEW && overview != null && overview.isEmpty()) {
                body = overview;
            }
            ...
            canApplyNow = mResponseJsonObject.optInt("isExpired");
            isOwner = mResponseJsonObject.optBoolean("isOwner");
            LogUtils.LOGV("logslogs", isOwner + " : owner");
            ...
            switch (mMLTViewType) {
                ...
                case ConstantVariables.CLASSIFIED_VIEW_WITH_CAROUSEL:
                    ...
                    if (mResponseJsonObject.has("isApplyJob") && canApplyNow == 0 && !isOwner) {
                ...
                case ConstantVariables.CLASSIFIED_VIEW_WITHOUT_CAROUSEL:
                    ...
                    if (!mResponseJsonObject.optString("price").equals("0") &&
                        ...
                        mProfileFieldsObject.put(mContext.getResources().getString(R.string.price),
                                CurrencyUtils.getCurrencyConvertedValue(mContext, mResponseJsonObject.optString("currency"),
            ...
            if (overview != null && !overview.isEmpty()) {
                ...
                overview = "<b>"  + mContext.getResources().getString(R.string.overview_text) + "</b> " + overview;

                    /* Setting Body in TextView */
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                    mOverviewContent.getSettings().setLayoutAlgorithm(WebSettings.LayoutAlgorithm.TEXT_AUTOSIZING);
                } else {
                    mOverviewContent.getSettings().setLayoutAlgorithm(WebSettings.LayoutAlgorithm.NORMAL);
                }
                GlobalFunctions.setWebSettings(mOverviewContent, false);
                mOverviewContent.loadDataWithBaseURL("file:///android_asset/", GlobalFunctions.getHtmlData(mContext, overview, true),
                        "text/html", "utf-8", null);

            } else {
                mOverviewView.setVisibility(View.GONE);
            }
