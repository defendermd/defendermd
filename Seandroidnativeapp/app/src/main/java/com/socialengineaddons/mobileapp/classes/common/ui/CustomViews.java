...
public class CustomViews {
    ...
    static OnLocationAndTagClickListener onLocationAndTagClickListener;
    ...
    public static void setOnLocationAndTagClickListener(OnLocationAndTagClickListener listener) {
        CustomViews.onLocationAndTagClickListener = listener;
    }
    ...
    /**
     * Method to return header view.
     * @param inflater LayoutInflater to inflate the header view layout.
     * @return Returns the inflated layout.
     */
    public static View getHeaderView(LayoutInflater inflater){
        return inflater.inflate(R.layout.load_comments_progress_item, null, false);
    }
    ...
    public static void setViewForLocationAndTaggedFriends(Context context, TextView tvLocationAndTaggedFriends,
        ...
        if (selectedFriends != null && !selectedFriends.isEmpty()) {
            if (selectedFriends.size() == 1) {
                tagFriendText = context.getResources().getString(R.string.with_text) + " <b>"
                        + selectedFriends.get(0) + "</b> ";
            } else if (selectedFriends.size() == 2) {
                tagFriendText = context.getResources().getString(R.string.with_text)
                        + " <b>" + selectedFriends.get(0) + "</b> " + context.getResources().getString(R.string.and)
                        + " <b>" + selectedFriends.get(1) + "</b> ";
            } else {
                tagFriendText = context.getResources().getString(R.string.with_text)
                        + " <b>" + selectedFriends.get(0) + "</b> " + context.getResources().getString(R.string.and)
                        + " <b>" + (selectedFriends.size() - 1) + " " + context.getResources().getString(R.string.others_tag_text) + "</b> ";
            }
            end = Html.fromHtml(tagFriendText.trim()).length();
        }

        if (location != null && !location.isEmpty()) {
            tagFriendText += context.getResources().getString(R.string.at) + " <b>" + location + "</b>";
        }

        CharSequence title = Html.fromHtml(tagFriendText);
        SpannableString text = new SpannableString(title);

        if (selectedFriends != null && !selectedFriends.isEmpty()) {
            tagStart = context.getResources().getString(R.string.with_text).length() + 1;
            ClickableSpan myClickableSpan = new ClickableSpan() {
                @Override
                public void onClick(View widget) {
                    if (onLocationAndTagClickListener != null) {
                        onLocationAndTagClickListener.onTagFriendClicked();
                    }
                }
            ...
            text.setSpan(myClickableSpan, tagStart, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            end++;
        }

        int locationStart = context.getResources().getString(R.string.at).length() + end + 1;
        if (location != null && !location.isEmpty()) {

            ClickableSpan locationClickableSpan = new ClickableSpan() {
                @Override
                public void onClick(View widget) {
                    if (onLocationAndTagClickListener != null) {
                        onLocationAndTagClickListener.onLocationClicked();
                    }
                }
            ...
            text.setSpan(locationClickableSpan, locationStart, title.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
        }

//        tvLocationAndTaggedFriends.setText(Html.fromHtml(tagFriendText));
        tvLocationAndTaggedFriends.setText(text);

        if ((selectedFriends == null || selectedFriends.isEmpty())
                && (location == null || location.isEmpty())) {
            tvLocationAndTaggedFriends.setVisibility(View.GONE);
        } else {
            tvLocationAndTaggedFriends.setVisibility(View.VISIBLE);
        }
    }

    public static RecyclerView getReactionPopupRecyclerView(Context mContext) {
        RecyclerView reactionsRecyclerView = new RecyclerView(mContext);

        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(mContext);
        linearLayoutManager.setOrientation(LinearLayoutManager.HORIZONTAL);

        reactionsRecyclerView.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT));
        ...
        reactionsRecyclerView.setBackground(ContextCompat.getDrawable(mContext, R.drawable.reaction_popup));
        reactionsRecyclerView.setPadding((int) mContext.getResources().getDimension(R.dimen.padding_7dp), 0,
                (int) mContext.getResources().getDimension(R.dimen.padding_7dp), 0);

        return reactionsRecyclerView;
    }

    public static LinearLayout getReactionPopupTipLinearLayout(Context mContext) {

        TextView textView = new TextView(mContext);
        textView.setBackground(mContext.getResources().getDrawable(R.drawable.ic_tip_down));
        textView.setLayoutParams(new LinearLayout.LayoutParams((int) mContext.getResources().getDimension(R.dimen.dimen_40dp),
                (int) mContext.getResources().getDimension(R.dimen.dimen_40dp)));
        textView.setGravity(Gravity.START);
        LinearLayout tipLayout = new LinearLayout(mContext);
        tipLayout.setOrientation(LinearLayout.HORIZONTAL);
        tipLayout.setBackgroundColor(mContext.getResources().getColor(R.color.transparent));
        tipLayout.setPadding((int) mContext.getResources().getDimension(R.dimen.reaction_tip_left_margin),
