...
import com.socialengineaddons.mobileapp.BuildConfig;
...
import com.socialengineaddons.mobileapp.classes.modules.friendrequests.FriendRequests;
...
import com.socialengineaddons.mobileapp.classes.modules.messages.MessageViewActivity;
...
import com.socialengineaddons.mobileapp.classes.modules.wishlist.ProductWishList;
import com.socialengineaddons.mobileapp.classes.modules.wishlist.WishListHomeFragment;
...
public class GlobalFunctions {
     ...
     */
    public static void updateAndroidSecurityProvider(Context callingActivity) {
        try {
            ProviderInstaller.installIfNeeded(callingActivity);
        } catch (GooglePlayServicesRepairableException e) {
            GooglePlayServicesUtil.showErrorDialogFragment(e.getConnectionStatusCode(), (Activity) callingActivity, null, 0, new DialogInterface.OnCancelListener() {
                @Override
    ...
    public static String getHtmlData(Context context, String bodyHTML, boolean isJustify) {
        ...
        if (bodyHTML != null && !bodyHTML.isEmpty()) {
            bodyHTML.replaceAll("\n\n", "<p>");
            if (bodyHTML.contains("<iframe src=\""))
                bodyHTML.replaceAll("<iframe src=\"", "<iframe src=");
        }
        if (isJustify) {
    ...
    public static Fragment getAuthenticateUserFragment(String currentSelectedModule,
        ...
        switch (currentSelectedModule) {
            case ConstantVariables.FORUM_MENU_TITLE:
                ...
                break;
            case ConstantVariables.HOME_MENU_TITLE:
                loadFragment = FeedsFragment.newInstance(null);
                break;
            case ConstantVariables.MLT_WISHLIST_MENU_TITLE:
                loadFragment = MLTUtil.getBrowseWishListPageInstance();
                break;
        ...
        if (itemName != null) {
            switch (itemName) {
                ...
                case ConstantVariables.ADV_VIDEO_PLAYLIST_MENU_TITLE:
                    icon = "\uf16a";
                    break;
                case ConstantVariables.ICON_TAG:
                    icon = "\uf02b";
                    break;
                case ConstantVariables.ICON_MAP_MARKER:
                    icon = "\uf041";
                    break;
                case ConstantVariables.ICON_EMPTY_HEART:
                    icon = "\uf08a";
                    break;
                case ConstantVariables.ICON_EMPTY_THUMBS_UP:
                    icon = "\uf164";
                    break;
                case ConstantVariables.ICON_SOLID_HEART:
                    icon = "\uf004";
                    break;
...
//        return Typeface.createFromAsset(context.getAssets(), "fontIcons/fontawesome-webfont.ttf");
    ...
    public static int getIdOfModule(JSONObject dataObject, String type) {
        ...
        switch (type) {
            ...
            case ConstantVariables.SITE_STORE_PRODUCT_MENU:
            ...
            case ConstantVariables.CROWD_FUNDING_MODULE_NAME:
            case ConstantVariables.CROWD_FUNDING_PROJECT_SUBJECT_TYPE:
            case ConstantVariables.CROWD_FUNDING_MAIN_TITLE:
                return dataObject.optInt("project_id");
            default:
                return 0;
        }
    }
    ...
    public static Intent getIntentForModule(Context context, int id, String type, String slug) {
        ...
        switch (type) {
            ...
            case ConstantVariables.ADV_VIDEO_MENU_TITLE:
            case ConstantVariables.ADV_VIDEO_TITLE:
            case ConstantVariables.ADV_SITE_VIDEO_TITLE:
                viewIntent = AdvVideoUtil.getViewPageIntent(context, id, null, bundle);
                break;

            case ConstantVariables.ADV_VIDEO_CHANNEL_MENU_TITLE:
            case ConstantVariables.SITE_VIDEO_CHANNEL_MENU_TITLE:
                slug = (id == 0) ? slug : String.valueOf(id);
                viewIntent = AdvVideoUtil.getChannelViewPageIntent(context, slug, url, bundle);
                break;
            ...
            case ConstantVariables.STORE_MENU_TITLE:
                slug = (id == 0) ? slug : String.valueOf(id);
                viewIntent = StoreUtil.getStoreViewPageIntent(context, slug);
                break;
            ...
            case ConstantVariables.SITE_PAGE_TITLE_MENU:
                slug = (id == 0) ? slug : String.valueOf(id);
                viewIntent = SitePageUtil.getViewPageIntent(context, slug, url, bundle);
                break;
            ...
            case ConstantVariables.ADVANCED_GROUPS_MENU_TITLE:
                slug = (id == 0) ? slug : String.valueOf(id);
                viewIntent = AdvGroupUtil.getViewPageIntent(context, slug, url, bundle);
                break;
            ...
            case ConstantVariables.CROWD_FUNDING_MAIN_TITLE:
            case ConstantVariables.CROWD_FUNDING_PROJECT_SUBJECT_TYPE:
                viewIntent = CoreUtil.getProjectViewPageIntent(context, String.valueOf(id));
                break;
            case ConstantVariables.USER_MENU_TITLE:
    ...
    public static String getFileRealPathFromUri(final Context context, final Uri uri) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && DocumentsContract.isDocumentUri(context, uri)) {
            ...
            //todo - for file uploading through "Downloads" folder, must not come inside this check.
    ...
    public static void openMessengerApp(Bundle conversationInfo, Context context, int openMessengerRequest) {

        try {
            JSONObject userDetail = new JSONObject(PreferencesUtils.getUserDetail(context));
            MessageCoreUtils.openChannelize(context, conversationInfo, openMessengerRequest);
        } catch (JSONException e) {
            e.printStackTrace();
        }
    }
    ...
    public static void classLoader(String fullClassPath) {
        try {
    ...
    public static boolean isViewer(Context context, int userId) {
        AppConstant appConstant = new AppConstant(context);
        if (!appConstant.isLoggedOutUser()) {
            JSONObject userDetails = null;
            try {
                userDetails = new JSONObject(PreferencesUtils.getUserDetail(context));
                if (userId == userDetails.optInt("user_id", 0)) {
                    return true;
                }
            } catch (JSONException e) {
                e.printStackTrace();
    ...
    public static String getAbsClassPath(Context context, String moduleName) {
        if (moduleName == null || (moduleName != null && moduleName.isEmpty())) {
            return null;
        }
        return context.getPackageName() + ".classes.modules." + moduleName + ".utils.CoreUtil";
    }
    ...
    public static void messageOwner(Context context, String currentSelectedModule,
        ...
        if (PreferencesUtils.isPrimeMessengerEnabled(context)) {
            MessageCoreUtils.openTwoUsersConversation(context, String.valueOf(browseList.getUserId()),
                    browseList.getUserDisplayName(),
                    browseList.getUserProfileImageUrl());
        } else {
            Intent intent;
    ...
    /*
    Images cropping ratio define across the app
    * User profile 16:9
    * Contest profile and sponsor logo 16:9
    * Participants entry images 1:1
    * Make appeal entry image 7:5
    * */

    public static UCrop getUCropInstance(Context mContext, String source, Uri destination) {
        UCrop uCrop = UCrop.of(Uri.fromFile(new File(source)), destination);
        ...
        options.setActiveWidgetColor(ContextCompat.getColor(mContext, R.color.green_blue));
        options.setToolbarWidgetColor(ContextCompat.getColor(mContext, R.color.textColorPrimary));
        options.setToolbarTitle(mContext.getResources().getString(R.string.crop_photo_text));
    ...
    /**
     * Method to invoke the class method from its path.
     * @param fullClassPath Full class path which contains the method.
     * @param method Method name which needs to be invoked.
     * @param args Arguments which needs to be passed in method.
     * @return Returns the Object instance.
     */
    public static Object invokeMethod(String fullClassPath, String method, Bundle args) {
        Object returnValue = null;
        if (fullClassPath == null) {
            return returnValue;
        }
        try {
            Class mClass = Class.forName(fullClassPath);
            Object object = mClass.newInstance();
            Method mMethod = mClass.getDeclaredMethod(method, Bundle.class);
            returnValue = mMethod.invoke(object, args);
            ...
            LogUtils.LOGD("Class Loader : InvocationTargetException ", e.getMessage()+" ");
        }
        return returnValue;
    }

    /**
     * Method to open the live preview screen with the passed parameters.
     * @param context Context of calling class.
     * @param subjectType Subject type if its opened from the user profile page of logged-in user.
     * @param subjectId Subject id i.e. logged-in user's id.
     * @param feedsFragment FeedFragment instance.
     */
    public static void openLivePreviewActivity(Context context, String subjectType, int subjectId, FeedsFragment feedsFragment) {
        Bundle liveStreamBundle = new Bundle();
        liveStreamBundle.putString(ConstantVariables.SUBJECT_TYPE, subjectType);
        liveStreamBundle.putInt(ConstantVariables.SUBJECT_ID, subjectId);
        Object object = GlobalFunctions.invokeMethod(context.getPackageName() + ".classes.modules.liveStreaming.LiveCoreUtils",
                "getLivePreviewIntent", liveStreamBundle);
        if (object != null && object instanceof Intent) {
            Intent intent = (Intent) object;
            intent.putExtras(liveStreamBundle);
            if (feedsFragment != null) {
                feedsFragment.startActivityForResult(intent, ConstantVariables.LIVE_VIDEO_REQUEST_CODE);
            } else {
                ((Activity) context).startActivityForResult(intent, ConstantVariables.LIVE_VIDEO_REQUEST_CODE);
            }
        }
    }

    /**
     * Method to open the live streaming/video view screen from the notifications/push notifications according to the status of live video.
     * @param context Context of calling class.
     * @param channelName Channel name for which live info needs to be fetched.
     * @param subjectType Type of the video (Core/site).
     * @param subjectId Id of the video(video-id).
     */
    public static void openLiveStreamActivity(Context context, String channelName, String subjectType,
                                              int subjectId) {
        ...
        progressDialog.show();
        AppConstant appConstant = new AppConstant(context);
        appConstant.getJsonResponseFromUrl(AppConstant.DEFAULT_URL
                + "livestreamingvideo/is-live?stream_name=" + channelName, new OnResponseListener() {
            @Override
            public void onTaskCompleted(JSONObject jsonObject) {
                progressDialog.dismiss();
                LogUtils.LOGD(GlobalFunctions.class.getSimpleName(), "jsonObject: " + jsonObject);
                if (jsonObject == null || jsonObject.length() == 0) {
                    Toast.makeText(context, context.getResources().getString(R.string.live_video_ended), Toast.LENGTH_SHORT).show();

                } else if (jsonObject.optInt("isLive") == 1) {
                    Bundle liveStreamBundle = new Bundle();
                    liveStreamBundle.putString(ConstantVariables.VIDEO_TYPE, subjectType);
                    liveStreamBundle.putInt(ConstantVariables.VIDEO_ID, subjectId);
                    liveStreamBundle.putString(ConstantVariables.SUBJECT_TYPE, subjectType);
                    liveStreamBundle.putInt(ConstantVariables.SUBJECT_ID, subjectId);
                    liveStreamBundle.putBoolean(ConstantVariables.IS_BROADCASTER, false);
                    liveStreamBundle.putString(ConstantVariables.CHANNEL_NAME, channelName);
                    Object object = GlobalFunctions.invokeMethod(context.getPackageName() + ".classes.modules.liveStreaming.LiveCoreUtils",
                            "getLiveStreamIntent", liveStreamBundle);
                    LogUtils.LOGD(GlobalFunctions.class.getSimpleName(), "object: " + object);
                    if (object != null && object instanceof Intent) {
                        Intent intent = (Intent) object;
                        intent.putExtras(liveStreamBundle);
                ...
                } else if (jsonObject.optString("endType").equals("normal")) {
                    Toast.makeText(context, context.getResources().getString(R.string.live_video_ended), Toast.LENGTH_SHORT).show();

                } else if (jsonObject.has("video_id") && jsonObject.has("videoUrl")) {
                    if (jsonObject.optString("videoUrl") != null && !jsonObject.optString("videoUrl").isEmpty()) {
                        int videoId = jsonObject.optInt("video_id");
                        String type = jsonObject.optString("subject_type");
                        Intent viewIntent = GlobalFunctions.getIntentForModule(context, videoId, type, null);
                        if (viewIntent != null) {
                            context.startActivity(viewIntent);
                            ((Activity) context).overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);
                        }
                    } else {
                        Toast.makeText(context, context.getResources().getString(R.string.video_available_soon), Toast.LENGTH_SHORT).show();
                    }
                } else {
                    Toast.makeText(context, context.getResources().getString(R.string.live_video_ended), Toast.LENGTH_SHORT).show();
                }
     ...
     /* Method to get drawable vector icon by name, here we can set multiple name
     *  to a single drawable as per use.
     * @param context
     * @param drawableName
     * @return
     */
    public static Drawable getDrawableByName(Context context, String drawableName) {
        ...
        switch (drawableName) {
            case "shipping":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_local_shipping_black_24dp).mutate();
                break;
            case "payment":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_payment_black_24dp).mutate();
                break;
            case "gender":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_gender).mutate();
                break;
            case "birthdate":
            case "gift":
            case "cake":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_birthday).mutate();
                break;
            case "user":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_profile).mutate();
                break;
            case "lock":
            case "key":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_password).mutate();
                break;
            case "email":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_email).mutate();
                break;
            case "setting":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_profile_type).mutate();
                break;
            case "country":
            case "city":
            case "location":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_city_country_location).mutate();
                break;
            case "interest":
            case "puzzle":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_interests).mutate();
                break;
            case "politics":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_political_opinion).mutate();
                break;
            case "religion":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_religion).mutate();
                break;
            case "zodiac":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_zodiac).mutate();
                break;
            case "dollar":
            case "currency":
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_dark_currency).mutate();
                break;
            default:
                drawable = ContextCompat.getDrawable(context, R.drawable.ic_field_icons_generic_fields).mutate();
        }
        if (drawable != null) {
            drawable.setColorFilter(new PorterDuffColorFilter(ContextCompat.getColor(context, R.color.grey),
                    PorterDuff.Mode.SRC_ATOP));
        }
        return drawable;
    }
     ...
     * @param context Context of calling class
     * @param isClearAll If we need to remove badge count from icon
     */
    public static void updateBadgeCount(Context context, boolean isClearAll) {
        if (!isClearAll) {
            String notificationCount = PreferencesUtils.getNotificationsCounts(context, PreferencesUtils.NOTIFICATION_COUNT);
            String messageCount = "0";
            if (PreferencesUtils.isPrimeMessengerEnabled(context)) {
                int missedCallCount = MessageCoreUtils.getMissedCallCount();
                messageCount = String.valueOf(PreferencesUtils.getUnReadMessageCount(context) + missedCallCount);
            }
            int count = Integer.valueOf(notificationCount) + Integer.valueOf(messageCount);
    ...
    public static CharSequence getHtmlModeText(String textString) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            return Html.fromHtml(textString, Html.FROM_HTML_MODE_COMPACT);
        } else {
            return Html.fromHtml(textString);
        }
    }

    /**
     * Method to check a service is currently running or not.
     *
     * @param context      Context of calling Activity.
     * @param serviceClass Service class instance.
     * @return Returns true, if the service is running currently otherwise false.
     */
