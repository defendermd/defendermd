...
public class ImageLoader {
    ...
    /***
     * Method to set people_suggestion_profile image from url into ImageView.
     *
     * @param url       Url which contains user image.
     * @param imageView Image view in which user image needs to be shown.
     */
    public void setImageForPeopleSuggestion(String url, ImageView imageView) {
        if (url != null && !url.isEmpty()) {
            Glide.with(mContext)
                    .setDefaultRequestOptions(getRequestOptions(R.drawable.user_default_image))
                    .load(url)
                    .into(imageView);
        } else {
            Glide.with(mContext)
    ...
    /***
     * Method to set album photo image from the given url.
     *
     * @param url       Url which contains user image.
     * @param imageView Image view in which user image needs to be shown.
     * @param randomColor randomColor which sets container background color.
     */
    public void setAlbumPhotosImage(String url, ImageView imageView, int randomColor) {
        if (url != null && !url.isEmpty()) {
    ...
    /***
     * Method to set image from the given url with the light grey color as a placeholder.
     *
     * @param url       Url which contains user image.
     * @param imageView Image view in which user image needs to be shown.
     */
    public void setLightGreyPlaceHolder(String url, ImageView imageView) {
        if (url != null && !url.isEmpty()) {
            /* Set image fit center if default image is coming from api */
            if (url.contains(ConstantVariables.DEFAULT_IMAGE_PATH)) {
                imageView.setScaleType(ImageView.ScaleType.FIT_CENTER);
    ...
    public void loadUserProfileImageWithPicasso(String url, ImageView imageView) {
        if (url != null && !url.isEmpty()) {
            Picasso.get()
                    .load(url)
