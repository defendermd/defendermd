...
public class SingleFeedPage extends AppCompatActivity implements View.OnClickListener, TextWatcher,
        AbsListView.OnScrollListener, OnFeedDisableCommentListener, OnAsyncResponseListener, OnCommentDeleteListener,
    ...
    LinearLayout mCommentPostBlock, mNoCommentsBlock;
    TextView mCommentPostButton, mCommentStickerPost, mNoCommentsImage;
    ...
    private ProgressBar mProgressBar, mSmallProgressBar;
    ...
    int mCommentId, mGetTotalItemCount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mIsMultiPhotoFeed) {
            mCommentListUrl = UrlUtil.AAF_LIKE_COMMNET_URL + "&action_id=" + mActionId + "&page=" + pageNumber + "&order=desc";

            mCommentAdapter = new CommentAdapter(this, R.layout.list_comment, mCommentListItems,
                    mCommentList, true, mSubjectType, mActionId, mActionId, false);
                            ...
                            public void onTaskCompleted(JSONObject jsonObject) throws JSONException {
                                ...
                                if (jsonObject != null) {
                                    mStickersPopup = StickersUtil.createStickersPopup(mContext, findViewById(R.id.commentList),
                                            ...
                                            jsonObject, mPhotoUploadingButton, mCommentStickerPost);
...
//                                    mAppConst.showKeyboard();
                            ...
                            SnackbarUtils.displaySnackbarLongWithListener(mCommentsListView,
                                    getResources().getString(R.string.user_does_not_have_access_resources), () -> {
                                        if (!isFinishing()) {
                                            finish();
                                        }
                                    });
                ...
                if (!isCommentsLoading && commentsObject != null && !mIsMultiPhotoFeed) {
                    try {
                        ...
                        mGetTotalItemCount = commentsObject.optInt("getTotalItemCount");
                        ...
                        if (mGetTotalComments != 0 || mGetTotalItemCount != 0) {
                        ...
                        // Scroll scrollview to the bottom if no comment is posted
    ...
    public void onClick(View view) {
        ...
        switch (id) {
            ...
            case R.id.loadMoreCommentText:
                ...
                pageNumber += 1;
                mCommentListUrl = UrlUtil.AAF_LIKE_COMMNET_URL + "&action_id=" + mActionId + "&page=" + pageNumber + "&order=desc";
                loadMoreComments(mCommentListUrl);
                break;
        ...
        try {
            if (commentsArray != null && commentsArray.length() != 0) {
                for (int i = 0; i < commentsArray.length(); i++) {
                    ...
                    mCommentListItems.add(0, new CommentList(userId, commentId, likeCount, isLike, authorPhoto, authorTitle,
                ...
                mCommentsListView.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (mSelectPath != null && mSelectPath.size() != 0) {
                            ...
                            new UploadFileToServerUtils(SingleFeedPage.this, mCommentPostUrl, mSelectPath,
                                    null, params).execute();

                        } else if (isReplycomment){
                            params.put("comment_id", String.valueOf(mCommentId));

                            mAppConst.postJsonResponseForUrl(UrlUtil.AAF_REPLY_URL, params, new OnResponseListener() {
                                @Override
                                public void onTaskCompleted(JSONObject jsonObject) throws JSONException {
    ...
    private void addCommentToList(JSONObject jsonObject) {
        ...
        if (jsonObject != null) {
            ...
            try {
                ...
                //for reply option in nested comment
            ...
            */
                ...
                if (!mIsMultiPhotoFeed && mFeedItemsList != null && mFeedItemsList.size() != 0) {
                    FeedList mSelectedFeed = (FeedList) mFeedItemsList.get(1);
                    mSelectedFeed.setmCommentCount(mSelectedFeed.getmCommentCount() + 1);
