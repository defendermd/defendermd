...
public class AdvEventsBrowseEventsFragment extends BaseFragment implements AdapterView.OnItemClickListener,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mGridView.setBackgroundColor(ContextCompat.getColor(mContext, R.color.white));
        ...
        if (getArguments() != null) {
            ...
            EVENT_TYPE = bundle.getString(AdvEventsUtil.EVENT_TYPE, "");

            if (EVENT_TYPE != null) {
                switch (EVENT_TYPE) {
                    case AdvEventsUtil.SEARCH_BY_DATE:
                        searchParams.clear();
        ...
        if (EVENT_TYPE != null && EVENT_TYPE.equals(AdvEventsUtil.BROWSE_EVENT)) {
        ...
        } else if (EVENT_TYPE != null && EVENT_TYPE.equals(AdvEventsUtil.CATEGORY_FILTER)) {
    ...
    public void onItemClick(AdapterView<?> adapterView, View view, int position, long l) {
        ...
        if (!isAllowedToView) {
            ...
            mainIntent.putExtra(AdvEventsProfilePage.OCCURRENCE_ID, listItems.getmOccurrenceId());
            startActivityForResult(mainIntent, ConstantVariables.VIEW_PAGE_CODE);
            getActivity().overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left);
        }
    ...
    public void sendRequestToServer() {
        ...
        try {
            ...
            /* Show data from cache only when page is opened form Browse Events tab */
            if (EVENT_TYPE.equals(AdvEventsUtil.BROWSE_EVENT) && (mEventTime.isEmpty() || mEventTime.equals("all"))) {
            ...
            mAppConst.getJsonResponseFromUrl(browseAdvancedEventsUrl, new OnResponseListener() {
                ...
                public void onTaskCompleted(JSONObject jsonObject) {
                    ...
                    /* Save data in cache only when page is opened form Browse Events tab */
                    if (EVENT_TYPE.equals(AdvEventsUtil.BROWSE_EVENT) && (mEventTime.isEmpty() || mEventTime.equals("all"))) {
            ...
            try {
                ...
                if (mDataResponse != null && mDataResponse.length() > 0) {
                    ...
                    for (int i = 0; i < mDataResponse.length(); i++) {
                        ...
                        int event_id = jsonDataObject.optInt("event_id");
                        int occurrence_id = jsonDataObject.optInt("occurrence_id");
                        String title = jsonDataObject.optString("title");
                        String location = jsonDataObject.optString("location");
                        String starttime = jsonDataObject.optString("starttime");

                        /* Replace events start date with current occurrence date and display in events list
                         * when user search events by date using calender */
                        if (searchParams != null && searchParams.containsKey("date_current")) {
                            starttime = searchParams.get("date_current") + " 00:00:00";
                        }
                        ...
                        String startPrice = jsonDataObject.optString("startFrom");

                        mBrowseItemList.add(new BrowseListItems(image_icon, hostImage, title,
                                hostTitle, member_count, event_id, (allow_to_view == 1), location,
                                starttime, hostId, hostType, hasMultipleDates, featured, sponsored, startPrice, occurrence_id));
                    ...
                    SnackbarUtils.displaySnackbarLongTime(rootView, message);
                    if (mFeaturedCount > 0) {
    ...
    private String getBrowseEventsUrl() {
        ...
        switch (EVENT_TYPE) {
            ...
            case AdvEventsUtil.CATEGORY_FILTER:
                ...
                break;
            case AdvEventsUtil.BROWSE_EVENT:
                if (!mEventTime.isEmpty()) {
