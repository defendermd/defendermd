        ...
        mBrowseDataAdapter.setOnQuantityChangeListener(new AdvEventsBrowseDataAdapter.OnQuantityChangeListener() {
            ...
            public void onDataChanged(int price, int count, int tickets_id, int isAdd) {
                ...
                if (isShowBookNow == 1) {
                    ...
                    mSubtotalTextView.setText(CurrencyUtils.getFormattedCurrencyString(currency, subTotal));
                    mDiscountTextView.setText(CurrencyUtils.getFormattedCurrencyString(currency, discount));

                    Double calculatedTax = (subTotal - discount) *(taxRate/100);
                    grandTotal = subTotal - discount + calculatedTax;

                    mTaxTextView.setText(CurrencyUtils.getFormattedCurrencyString(currency, calculatedTax));
                    mGrandTotalTextView.setText(CurrencyUtils.getFormattedCurrencyString(currency, grandTotal));
                }
            }
        });
