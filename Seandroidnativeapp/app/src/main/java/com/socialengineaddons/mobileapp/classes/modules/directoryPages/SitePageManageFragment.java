 ...
 */
public class SitePageManageFragment extends BaseFragment implements AdapterView.OnItemClickListener,
    ...
    String mManagePageUrl, mCurrentSelectedModule;
    private SwipeRefreshLayout swipeRefreshLayout;
    private boolean isLoading = false, isVisibleToUser = false, isAdLoaded = false, isCommunityAds = false;
    Snackbar snackbar;
    private NativeAdsManager listNativeAdsManager;

    public static SitePageManageFragment newInstance(Bundle bundle) {
        SitePageManageFragment fragment = new SitePageManageFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mManagePageUrl = UrlUtil.MANAGE_SITE_PAGE_URL + "&page=" + pageNumber;

        mManageDataAdapter = new ManageDataAdapter(getActivity(), R.layout.page_item_view, mBrowseItemList,
    ...
    public void setMenuVisibility(final boolean visible) {
        ...
        if (visible && !isVisibleToUser) {
            if(ConstantVariables.SITE_PAGE_ADS_TYPE == ConstantVariables.TYPE_GOOGLE_ADS){
                mAdFetcher = new AdFetcher(mContext);
                mAdFetcher.loadAds(mBrowseItemList,mManageDataAdapter,ConstantVariables.SITE_PAGE_ADS_POSITION);
            }
            sendRequestToServer();
        } else {
    ...
    public void sendRequestToServer(){
        ...
        mAppConst.getJsonResponseFromUrl(mManagePageUrl, new OnResponseListener() {
            @Override
    ...
    public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
        ...
        if(limit == totalItemCount && !isLoading) {
            ...
            if(limit >= AppConstant.LIMIT && (AppConstant.LIMIT * mLoadingPageNo) <
                ...
                String url = mManagePageUrl = UrlUtil.MANAGE_SITE_PAGE_URL + "&page=" + mLoadingPageNo;
                isLoading = true;
                loadMoreData(url);
            }
        }
    ...
    private void addDataToList(JSONObject jsonObject) {
        ...
        if (mBody != null) {
            ...
            mDataResponse = mBody.optJSONArray("response");
                ...
                if (mDataResponse != null && mDataResponse.length() > 0) {
                    ...
                    String distance = jsonDataObject.optString("distance");
                        ...
                        mBrowseItemList.add(new BrowseListItems(image_icon, title,
                            like_count, follow_count, page_id,
                            jsonDataObject.optJSONArray("menu"), location, distance, review_count, rating));
                }
            } else {
                ...
                errorIcon.setText("\uf15c");
                errorMessage.setText(getActivity().getResources().getString(R.string.no_pages));
            }
