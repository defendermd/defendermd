...
package com.socialengineaddons.mobileapp.classes.modules.messages;
...
import com.socialengineaddons.mobileapp.classes.common.interfaces.OnSuccessListener;
...
import com.socialengineaddons.mobileapp.classes.common.utils.AttachmentDialogUtil;
...
public class CreateNewMessage extends AppCompatActivity implements TextWatcher, AdapterView.OnItemClickListener,
    ...
    private boolean isSendMessageRequest = false, isStoryReply = false, isSendMsgInMessenger = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog.setMessage(getResources().getString(R.string.progress_dialog_wait) + "…");
        ...
        isSendMsgInMessenger = getIntent().getBooleanExtra(ConstantVariables.SEND_MESSAGE_IN_MESSENGER, false);
        ...
        alertDialogPositiveButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!linkText.isEmpty() && GlobalFunctions.isValidUrl(linkText)) {
                    ...
                    alertDialog.dismiss();
                    mAppConst.hideKeyboard();
                    new UploadFileToServerUtils(CreateNewMessage.this, mAttachUrl, uriText, mAttachType).execute();
                } else {
            ...
            case ConstantVariables.REQUEST_MUSIC:
                if(resultCode == RESULT_OK && data != null){
                    ...
                    if (selectedMusicUri != null) {
                        ...
                        new UploadFileToServerUtils(CreateNewMessage.this, mAttachUrl, mSelectedMusicFile).execute();
                    }
        ...
        try {
            disposable = EventBus.getInstance().getEvents().subscribe(event -> {
                switch (event.getEvent()) {
                    case Event.EVENT_SEND_MESSAGE_SUCCESS:
                    ...
                    case Event.EVENT_SEND_MESSAGE_ERROR:
                        if (progressDialog.isShowing()) {
                            progressDialog.dismiss();
                        }
                        SnackbarUtils.displaySnackbar(findViewById(R.id.create_form),
                                getResources().getString(R.string.please_retry_option));
                        break;
                }
            }, throwable -> {
                LogUtils.LOGD(CreateNewMessage.class.getSimpleName(), "throwable: "+throwable);
            });
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void sendMessageInMessenger() {
        ...
        checkValidation();
        if (!isBodyBlank) {
            String message = mBodyView.getText().toString();
            if (isStoryReply) {
                message = getResources().getString(R.string.replied_to_story) + ": \n" + message;
            }
            progressDialog.show();
            MessageCoreUtils.sendMessageToUser(String.valueOf(sendToUserId),
                    message);
        }
    }
        ...
        if(!isBodyBlank && !isRecipientBlank){
            ...
            try {
                ...
                if (mAttachType != null && !mAttachType.isEmpty()) {
                    mPostParams = UploadFileToServerUtils.getAttachmentPostParams(mPostParams, mAttachType,
                            uriText, mSongId, mVideoId);
                }

                // Uploading files in background with the Message details.
                new UploadFileToServerUtils(CreateNewMessage.this, createFormUrl, mPostParams,
                        mSelectPath).execute();
