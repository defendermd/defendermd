...
import com.socialengineaddons.mobileapp.classes.modules.user.MemberClusterItems;
...
public class BrowseMLTFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,
    ...
    public void makeRequest() {
        ...
        mAppConst.getJsonResponseFromUrl(mBrowseMLTUrl, new OnResponseListener() {
            ...
            public void onTaskCompleted(JSONObject jsonObject) {
                ...
                if (!isViewTypeLoaded.contains(String.valueOf(mViewType)) && !isSearchTextSubmitted && getActivity() != null) {
    ...
    public void addDataToMap(JSONObject response) {
        ...
        if (mDataResponse != null && mDataResponse.length() > 0) {
            ...
            // Show End of Result Message when there are less results
            if (mapItemCount > AppConstant.LIMIT * mLoadingPageNo) {
    ...
    public void setLayoutManager(final int mMLTBrowseType) {
        try {
            LogUtils.LOGD(BrowseMLTFragment.class.getSimpleName(), "setLayoutManager: "+mMLTBrowseType);
            ...
            if (mViewType != 4) {
                if (!mAppConst.isLoggedOutUser() && !isCategoryResults &&
    ...
    private void readyToViewMore() {
        if ((AppConstant.LIMIT * mLoadingMapPageNo) < mapItemCount) {
            ...
            SnackbarUtils.displaySnackbar(mRootView, mContext.getResources().getString(R.string.no_more_item_display));
        }
    }
