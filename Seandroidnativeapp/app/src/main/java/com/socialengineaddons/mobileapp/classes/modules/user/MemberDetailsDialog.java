...
public class MemberDetailsDialog extends DialogFragment {
    ...
    private boolean mIsNeedToDismissDialog = false, isLoading = false,
            isNeedToShowAdminApprovalMessage = false, isLiveStreamViewer = false;
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        ...
        if (getArguments() != null) {
            ...
            isLiveStreamViewer = bundle.getBoolean(ConstantVariables.IS_LIVE_STREAM_VIEWER, false);
            if (isLiveStreamViewer && bundle.containsKey(ConstantVariables.LIVE_STREAM_VIEWERS_LIST)) {
                List<MemberDetails> listItems = (List<MemberDetails>) bundle.getSerializable(ConstantVariables.LIVE_STREAM_VIEWERS_LIST);
                if (listItems != null && !listItems.isEmpty()) {
                    mBrowseItemList.clear();
                    mBrowseItemList.addAll(listItems);
                }
            }
        }
        ...
        if (isLiveStreamViewer) {
            if (mBrowseItemList.size() == 1) {
                layoutParams.height = mContext.getResources().getDimensionPixelSize(R.dimen.dimen_230dp);
            } else {
                layoutParams.height = AppConstant.getDisplayMetricsHeight(mContext) / 2;
            }
        } else {
    ...
    public void initializeViews() {
        ...
        mMemberDetailsAdapter = new MemberDetailsAdapter(mContext, isLiveStreamViewer, mBrowseItemList, MemberDetailsDialog.this);
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (limit == totalItemCount && !isLoading) {
                    ...
                    if (limit >= AppConstant.LIMIT && (AppConstant.LIMIT* mLoadingPageNo)
                            < memberDetails.getTotalItemCount()) {
    ...
    public void addDataToList(JSONObject jsonObject) {
        ...
        memberDetails.setTotalItemCount(jsonObject.optInt("totalItemCount"));
        if (responseArray != null && responseArray.length() > 0) {
            ...
            for (int i = 0; i < responseArray.length(); i++) {
                ...
                mBrowseItemList.add(new MemberDetails(userId, userName, userImage, commentDescription));
            }
    ...
    private void addAdminApprovalMessage() {
        if (isNeedToShowAdminApprovalMessage && PreferencesUtils.getUserDetail(mContext) != null) {
            try {
                ...
                mBrowseItemList.add(0, new MemberDetails(userDetail.getInt("user_id"), userDetail.optString("displayname"),
