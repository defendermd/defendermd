//
//  AdvancedVideoSearchViewController.swift
//  seiosnativeapp
//
...
class AdvancedVideoSearchViewController: UIViewController , UITableViewDataSource, UITableViewDelegate , UISearchBarDelegate, TTTAttributedLabelDelegate,WKNavigationDelegate {
...
//        searchBar.setTextColor(textColorPrime)
        ...
        searchBar.setPlaceholderWithColor(NSLocalizedString("Search Videos",  comment: ""))
        searchBar.delegate = self
        
        let leftNavView = UIView(frame: CGRect(x: 0, y: 0, width: 44, height: 44))
        leftNavView.backgroundColor = UIColor.clear
        let tapView = UITapGestureRecognizer(target: self, action: #selector(AdvancedVideoSearchViewController.cancel))
        ...
        let filter = UIBarButtonItem( title: fiterIcon , style: UIBarButtonItem.Style.plain , target:self , action: #selector(AdvancedVideoSearchViewController.filter))
        ...
        videoTableView = UITableView(frame: CGRect(x: 0, y: TOPPADING+5,width: view.bounds.width, height: view.bounds.height - tabBarHeight - (TOPPADING + 5)), style: .grouped)
        ...
        if #available(iOS 11.0, *) {
            videoTableView.estimatedRowHeight = 0
            videoTableView.estimatedSectionHeaderHeight = 0
            videoTableView.estimatedSectionFooterHeight = 0
        }
   ...
   // MARK: - Cancel
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell
    {
        ...
        //Select Video Action
        cell.contentSelection.addTarget(self, action: #selector(AdvancedVideoSearchViewController.showVideo(_:)), for: .touchUpInside)
            
        cell.imgVideo.addTarget(self, action: #selector(AdvancedVideoSearchViewController.btnVideoIconClicked(_:)), for: .touchUpInside)
        ...
        // Set Video Image
        if (videosInfo["photo_id"] as? Int) != nil{
        ...
        if(isIpad()){
            ...
            // Select Video Action
            cell.contentSelection2.addTarget(self, action: #selector(AdvancedVideoSearchViewController.showVideo(_:)), for: .touchUpInside)
            cell.imgVideo2.addTarget(self, action: #selector(AdvancedVideoSearchViewController.btnVideoIconClicked(_:)), for: .touchUpInside)
                ...
                DispatchQueue.main.async(execute: {
                    ...
                    if msg{
                        ...
                        if self.videosResponse.count == 0{
                            ...
                            showToast(message: NSLocalizedString("You do not have any video entries. Get started by creating a new entry.", comment: ""), controller: self)
