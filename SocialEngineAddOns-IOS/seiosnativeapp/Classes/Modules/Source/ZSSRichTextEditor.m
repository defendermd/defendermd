...
- (void)viewDidLoad {
    ...
    CGRect frame = CGRectMake(6, 70, self.view.frame.size.width - 17, (self.view.frame.size.height - 64));
...
//    self.editorView.scrollView.scrollEnabled = YES;
//    self.editorView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin;
//    self.editorView.dataDetectorTypes = UIDataDetectorTypeAll;
...
- (void)buildToolbar {
    ...
    if (items.count == 0 && !(_enabledToolbarItems && [_enabledToolbarItems containsObject:ZSSRichTextEditorToolbarNone])) {
      //  _enabledToolbarItems = @[ZSSRichTextEditorToolbarAll];
        ...
        self.enabledToolbarItems = @[ZSSRichTextEditorToolbarBold, ZSSRichTextEditorToolbarItalic, ZSSRichTextEditorToolbarParagraph, ZSSRichTextEditorToolbarUnorderedList,ZSSRichTextEditorToolbarOrderedList,ZSSRichTextEditorToolbarInsertLink,ZSSRichTextEditorToolbarJustifyLeft,ZSSRichTextEditorToolbarJustifyCenter,ZSSRichTextEditorToolbarJustifyRight,ZSSRichTextEditorToolbarJustifyFull,ZSSRichTextEditorToolbarHorizontalRule,ZSSRichTextEditorToolbarIndent,ZSSRichTextEditorToolbarOutdent,ZSSRichTextEditorToolbarViewSource];
...
- (void)updateHTML {
    ...
    NSString *value = [[NSUserDefaults standardUserDefaults] stringForKey:@"blogandclassified"];
    
    if (value != nil && ![value  isEqual: @""]) {
        
        NSInteger myLength = [html length];
        if (myLength >=1 ) {
            
            
            [[NSNotificationCenter defaultCenter]
             postNotificationName:@"texttypecheck"
             object:self];
...
- (void)getText:(void (^ _Nullable)(_Nullable id, NSError * _Nullable error))completionHandler {
    ...
    [self.editorView evaluateJavaScript:ZSSEditorText completionHandler:^(NSString *result, NSError *error) {
        ...
        if (value != nil && ![value  isEqual: @""]) {
            ...
            NSInteger myLength = [result length];
            if (myLength <= 0) {
                
                
                [[NSNotificationCenter defaultCenter]
                 postNotificationName:@"texttypecheckzero"
                 object:self];
...
- (void)showHTMLSource:(ZSSBarButtonItem *)barButtonItem {
    if (self.sourceView.hidden) {
        ...
        barButtonItem.tintColor = [self barButtonItemDefaultColor];
         //barButtonItem.backgroundColor = [self barButtonItemDefaultColor];
        self.sourceView.hidden = YES;
...
- (void)showInsertLinkDialogWithLink:(NSString *)url title:(NSString *)title {
    ...
    if ([NSProcessInfo instancesRespondToSelector:@selector(isOperatingSystemAtLeastVersion:)]) {
        ...
        [alertController addTextFieldWithConfigurationHandler:^(UITextField *textField) {
            ...
            textField.text = @"http://";
            if (url) {
               // NSString *myString = [NSString stringWithFormat:@"%@%@", @"http://", linkURL.text];
                NSString *myString = [NSString stringWithFormat:@"%@", url];
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:insertButtonTitle style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            ...
            if (!self.selectedLinkURL) {
                 ...
                 NSString *myString = [NSString stringWithFormat:@"%@",linkURL.text];
                
                [self insertLink:myString title:title.text];
                //NSLog(@"insert link");
            } else {
...
- (void)keyboardWillShowOrHide:(NSNotification *)notification {
    ...
    if ([notification.name isEqualToString:UIKeyboardWillShowNotification]) {
        ...
        [UIView animateWithDuration:duration delay:0 options:animationOptions animations:^{
            ...
            editorFrame.size.height = ((self.view.frame.size.height - keyboardHeight - 60) - sizeOfToolbar - extraHeight) ;
            ...
            sourceFrame.size.height = (self.view.frame.size.height - keyboardHeight - 64) - sizeOfToolbar - extraHeight;
...
- (UIColor *)barButtonItemSelectedDefaultColor {
    ...
    //return [UIColor blackColor];
     return [UIColor colorWithRed:80.0f/255.0f green:164.045f/255.0f blue:253.423f/255.0f alpha:1.0f];
