...
public struct TLPhotosPickerConfigure {
    public var defaultCameraRollTitle = NSLocalizedString("Gallery",comment: "")
...
extension TLPhotosPickerViewController {
    ...
    @objc open func makeUI() {
        ...
        self.subTitleArrowImageView.image = TLBundle.podBundleImage(named: "arrow")!.maskWithColor(color: .white)
    ...
    open func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = (info[UIImagePickerController.InfoKey.originalImage] as? UIImage)
        {
            ...
            PHPhotoLibrary.shared().performChanges({
                ...
                // get the asset
                let asset = AVURLAsset(url: info[UIImagePickerController.InfoKey.mediaURL] as! URL)
...
extension TLPhotosPickerViewController: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDataSourcePrefetching {
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let index = self.selectedAssets.index(where: { $0.phAsset == asset.phAsset }) {
            ...
            if checkTypeInArray(asset: asset) == false
            {
                asset.selectedOrder = self.selectedAssets.count - 1
                self.selectedAssets.removeLast()
                cell.selectedAsset = false
    ...
    func checkTypeInArray(asset:TLPHAsset)-> Bool
    {
        if let asset = self.selectedAssets.last {
            ...
            if asset.type == .video {
                ...
                {
                    let alert = UIAlertController(title: "", message: NSLocalizedString("Video can not be selected with image",comment: ""), preferredStyle: .alert)
