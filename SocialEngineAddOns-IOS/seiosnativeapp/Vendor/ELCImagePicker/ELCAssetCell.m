...
@property (nonatomic, strong) NSMutableArray *cacheAssest;
//cachAssest
    ...
    if(!IS_IOS8){
        ...
        for (int i = 0; i< [_rowAssets count]; ++i)
        {
           ...
           PHAsset *phAsset = (PHAsset *)asset.asset;
            [_cacheAssest addObject:phAsset];
            
        }
        
        [self.imageManager startCachingImagesForAssets:_cacheAssest targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFill options:nil];
        ...
        for (int i = 0; i < [_rowAssets count]; ++i) {
            ...
            //[self.imageManager startCachingImagesForAssets:assets targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFill options:nil];
            ...
            if (i < [_imageViewArray count]) {
                ...
                [self.imageManager requestImageForAsset:phAsset targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFill options:options resultHandler:^(UIImage * result, NSDictionary * info) {
                    imageView.image = nil;
                ...
                imageView.image = nil;
                PHAsset *phAsset = (PHAsset *)asset.asset;
                [self.imageManager requestImageForAsset:phAsset targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFit options:options resultHandler:^(UIImage * result, NSDictionary * info) {
                    imageView.image = nil;
                    imageView.image = result;
                    imageView.contentMode = UIViewContentModeScaleAspectFit;
                }];
...
- (void)cellTapped:(UITapGestureRecognizer *)tapRecognizer
{
	...
	CGRect frame = CGRectMake(startX, 2, (self.bounds.size.width/3)-3, (self.bounds.size.width/3));
...
- (void)layoutSubviews
{
    ...
    CGRect newFrame = UIEdgeInsetsInsetRect(self.layer.frame, UIEdgeInsetsMake(4, 5, 4, 5));
