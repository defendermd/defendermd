...
NSString *const FXFormFieldTypeRate = @"rateview";
NSString *const FXFormFieldTypeImage = @"image";
NSString *const FXFormFieldTypeHelp = @"help";
NSString *const FXFormFieldTypeSwitch = @"switch";
...
static Class FXFormFieldInferClass(NSDictionary *dictionary)
{
    ...
    return @{FXFormFieldTypeLabel: [NSString class],
             ...
             FXFormFieldTypeRate: [NSString class],
             FXFormFieldTypeImage: [UIImage class],
             FXFormFieldTypeHelp: [NSString class],
             FXFormFieldTypeSwitch: [NSNumber class]
             }[type];
}
...
- (id)value
{
    if (FXFormCanGetValueForKey(self.form, self.key))
    {
        ...
        //NSMutableString *str = [(NSObject *)self.form valueForKey:self.key];
        if (value && self.options)
        {
            if ([self isIndexedType])
            {
        ...
        //        if([str containsString:@"seiosnativeapp.CreateNewForm:"])
        //        {
        //            value = @"";
        //        }
        return value;
...
- (instancetype)init
{
    if ((self = [super init]))
    {
        ...
        _cellClassesForFieldTypes = [@{FXFormFieldTypeDefault: [FXFormDefaultCell class],
                                       FXFormFieldTypeLabel: [FXFormDefaultCell class],
                                       FXFormFieldTypeHelp: [FXFormDefaultCell class],
                                       ...
                                       FXFormFieldTypeRate: [FXFormstarviewCell class],
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section
{
    if([view isKindOfClass:[UITableViewHeaderFooterView class]])
    {
        ...
        NSString *str = [tableViewHeaderFooterView.textLabel.text lowercaseString];
        ...
        if (str && str.length > 0) {
            ...
            capitalisedSentence = [str stringByReplacingCharactersInRange:NSMakeRange(0,1)
                                                               withString:[[str substringToIndex:1] capitalizedString]];
        } else {
            // No, it doesn't.
        }
        tableViewHeaderFooterView.textLabel.text = capitalisedSentence;
    }
}
...
- (CGFloat)tableView:(__unused UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *array = [FXFormCustomization fxHideIndex];
    for (id object in array) {
        if ([object intValue] == (int)rowNumber){
            return 0.0001;
            break;
        }
    }
...
- (UITableViewCell *)tableView:(__unused UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.tintColor = [FXFormCustomization fxbuttonColor];
    NSInteger rowNumber = 0;
    for (NSInteger i = 0; i < indexPath.section; i++)
    {
        rowNumber += [self tableView:tableView numberOfRowsInSection:i];
    ...
    for (id object in array)
    {
        if ([object integerValue] == (int) rowNumber)
        {
            cell.hidden = true;
            break;
        }
    ...
    return  cell;//[self cellForField:[self fieldForIndexPath:indexPath]];
}
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    {
        self.textLabel.font = [UIFont fontWithName:[FXFormCustomization fxfontName] size:[FXFormCustomization fxfontSizeNormal]];// [UIFont boldSystemFontOfSize:17];
        FXFormLabelSetMinFontSize(self.textLabel, FXFormFieldMinFontSize);
        self.detailTextLabel.font = [UIFont fontWithName:[FXFormCustomization fxfontName] size:[FXFormCustomization fxfontSizeNormal]];//[UIFont systemFontOfSize:17];
...
- (void)update
{
    ...
    if ([[self.field.type uppercaseString] isEqualToString:[FXFormFieldTypeLabel uppercaseString]])
    {
        ...
        if (self.field.value != nil)
        {
            ...
            if ([self.field.value isEqual: @0])
            {
                imageView.image = [UIImage imageNamed:@"downArrow.png"];
            }
            else
            {
               imageView.image = [UIImage imageNamed:@"upArrow.png"];
            }
            imageView.clipsToBounds = YES;
            [imageView setFrame:CGRectMake(self.frame.size.width-40,0,25,25)];
            self.accessoryView = imageView;
            self.detailTextLabel.text = nil;
            self.textLabel.font = [UIFont fontWithName:@"Avenir-Book" size:17];
            self.textLabel.textColor = [UIColor colorWithRed:60/255.0 green:60/255.0 blue:60/255.0 alpha:1.0];
    ...
    else if ([[self.field.type uppercaseString] isEqualToString:[FXFormFieldTypeSwitch uppercaseString]])
    {
        ...
        else if ([self.field.value  isEqual: @1])
        {
            [switchView setOn:YES];
        }
        
        [switchView setFrame:CGRectMake(self.frame.size.width-40,0,25,25)];
        self.accessoryView = switchView;
        self.selectionStyle = UITableViewCellSelectionStyleNone;
        //self.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
    else if ([[self.field.type uppercaseString] isEqualToString:[FXFormFieldTypeHelp uppercaseString]])
    {
        UIImageView *imageView = [[UIImageView alloc] init];
        imageView.contentMode = UIViewContentModeScaleAspectFill;
        imageView.image = [UIImage imageNamed:@"info2.png"];
        imageView.clipsToBounds = YES;
        [imageView setFrame:CGRectMake(self.frame.size.width-40,0,20,20)];
        self.accessoryView = imageView;
        //self.accessoryType = UITableViewCellAccessoryDisclosureIndicator;
    }
...
- (void)didSelectWithTableView:(UITableView *)tableView controller:(UIViewController *)controller
{
    if ([self.field.type isEqualToString:FXFormFieldTypeBoolean] || [self.field.type isEqualToString:FXFormFieldTypeOption])
    {
        ...
        if ([self.field.type isEqualToString:FXFormFieldTypeOption])
        {
            ...
            if (indexPath)
            {
                ...
                //                [tableView reloadSections:[NSIndexSet indexSetWithIndex:indexPath.section] withRowAnimation:UITableView.RowAnimationAutomatic];
            }
        }
        else
        {
    ...
    else if ([[self.field.type uppercaseString] isEqualToString:[FXFormFieldTypeLabel uppercaseString]])
    {
        if (self.field.value != nil)
        {
            if ([self.field.value isEqual: @0])
...
- (void)setUp
{
    ...
    self.textField.font = [UIFont fontWithName:[FXFormCustomization fxfontName] size:[FXFormCustomization fxfontSizeSmall]];//[UIFont systemFontOfSize:self.textLabel.font.pointSize];
...
+ (CGFloat)heightForField:(FXFormField *)field width:(CGFloat)width
{
    ...
    dispatch_once(&onceToken, ^{
        ...
        textView.font = [UIFont fontWithName:[FXFormCustomization fxfontName] size:[FXFormCustomization fxfontSizeNormal]];//[UIFont systemFontOfSize:17];
    });
    ...
    if (value != nil && ![value  isEqual: @""] ){
        ...
        if (textView.text != nil && ![textView.text  isEqual: @""]) {
            ...
            [[NSUserDefaults standardUserDefaults] setObject:textView.text forKey:@"preferenceName"];
...
- (void)setUp
{
    ...
    self.textView = [[UITextView alloc] initWithFrame:CGRectMake(0, 0, 320, 42)];
    ...
    self.textView.font = [UIFont fontWithName:[FXFormCustomization fxfontName] size:[FXFormCustomization fxfontSizeNormal]];//[UIFont systemFontOfSize:17];
...
- (void)update
{
    ...
    NSString *htmlString = [self.field fieldDescription ];
    if (htmlString.length != 0)
    {
        NSString *value = [[NSUserDefaults standardUserDefaults] stringForKey:@"SellSomething"];
    ...
    self.textView.attributedText = attributedString; //[self.field fieldDescription ];
...
{
    //defaults.setObject("Coding Explorer", forKey: "forumcheckkey")
    
    NSString *value = [[NSUserDefaults standardUserDefaults] stringForKey:@"forumcheckkey"];
    
    if (value != nil && ![value  isEqual: @""] ){
        [[NSNotificationCenter defaultCenter]
         postNotificationName:@"ScrollingactionPage1"
         object:self];
    }
    //    let obj1 = NSNotificationCenter.defaultCenter();
    //    obj1.postBodyName("callbody", object: nil);
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if (value != nil && ![value  isEqual: @""] ){
        ...
        //scroll to show cursor
        CGRect cursorRect = [self.textView caretRectForPosition:self.textView.selectedTextRange.end];
        [tableView scrollRectToVisible:[tableView convertRect:cursorRect fromView:self.textView] animated:false];
...
-(void)changeDateFromButton:(id)sender
{
...
//        {
            ...
            doneView=[[UIView alloc]initWithFrame:CGRectMake(0, self.window.bounds.size.height - self.datePicker.frame.size.height-30, self.window.bounds.size.width, 50)];
            [doneView setBackgroundColor:[UIColor lightGrayColor]];
            
            [doneView setUserInteractionEnabled:YES];
            [doneView setTag:8];
            [currentWindow addSubview:doneView];
...
{
    self.textLabel.text = self.field.title;
    //     if ([[FXFormCustomization fxMediaType]  isEqual: @"video"] ){
    //         @autoreleasepool {
    ...
    //
    //         }
    //         [[ALAssetsLibrary new] assetForURL:self.field.value resultBlock:^(ALAsset *asset) {
    //             NSLog(@"Image %@", [UIImage imageWithCGImage:asset.thumbnail]);
    //              self.imagePickerView.image = [UIImage imageWithCGImage:asset.thumbnail];
...
- (UIImage *)imageValue
{
    if (self.field.value)
    {
        if ([[FXFormCustomization fxMediaType]  isEqual: @"video"] ){
            @autoreleasepool {
                AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:self.field.value options:nil];
                AVAssetImageGenerator *gen = [[AVAssetImageGenerator alloc] initWithAsset:asset];
                gen.appliesPreferredTrackTransform = YES;
                CMTime time = CMTimeMakeWithSeconds(0.0, 600);
                NSError *error = nil;
    ...
    else if ([[NSUserDefaults standardUserDefaults] objectForKey:@"imagePicked"] != nil) {
        NSData* imageData = [[NSUserDefaults standardUserDefaults] objectForKey:@"imagePicked"];
...
- (void)didSelectWithTableView:(UITableView *)tableView controller:(UIViewController *)controller
{
    ...
    NSString *option = NSLocalizedString(@"Take Photo", nil);
    if ([ [FXFormCustomization fxMediaType]  isEqual: @"video"] ){
        option = NSLocalizedString(@"Shoot Video", nil);
    }
    ...
    else if ([UIAlertController class])
    {
        ...
        [alert addAction:[UIAlertAction actionWithTitle:option style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
            [self didDismissWithButtonIndex:0];
        ...
        [alert addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Photo Library", nil) style:UIAlertActionStyleDefault handler:^(__unused UIAlertAction *action) {
            [self didDismissWithButtonIndex:1];
        }]];
    ...
    {
        self.controller = controller;
        [[[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", nil) destructiveButtonTitle:nil otherButtonTitles:option, NSLocalizedString(@"Photo Library", nil), nil] showInView:controller.view];
    }
}
...
{
    if ([ [FXFormCustomization fxMediaType]  isEqual: @"video"] ){
        self.field.value = info[UIImagePickerControllerMediaURL];
        NSURL *videoURL=[info objectForKey:@"UIImagePickerControllerMediaURL"];
        AVURLAsset *asset = [[AVURLAsset alloc] initWithURL:videoURL options:nil];
        double durationInSeconds = 0.0;
        if (asset)
        durationInSeconds = CMTimeGetSeconds(asset.duration);
        NSNumber *myDoubleNumber = [NSNumber numberWithDouble:durationInSeconds];
        [[NSUserDefaults standardUserDefaults] setObject:[myDoubleNumber stringValue] forKey:@"videoDurationKey"];
    }else{
        ...
        [[NSUserDefaults standardUserDefaults] setObject:UIImagePNGRepresentation(self.field.value) forKey:@"imagePicked"];
    }
    [picker dismissViewControllerAnimated:YES completion:NULL];
    if (self.field.action) self.field.action(self);
    [self update];
...
//        appDelegate.window.rootViewController = self.controller;
...
@interface FXFormstarviewCell () <FloatRatingViewDelegate>
@property (nonatomic, strong) FloatRatingView *viewrating;
@end
@implementation FXFormstarviewCell
...
- (void)setUp
{
    ...
    self.viewrating = [[FloatRatingView alloc] initWithFrame:CGRectMake(screenWidth - 165, 10, 150, 21)];
    self.viewrating.emptyImage = [UIImage imageNamed:@"graystar.png"];
    self.viewrating.fullImage = [UIImage imageNamed:@"yellowStar.png"];
    // Optional params
    ...
    //self.viewrating.rating = 2.5
    NSLog(@"%ld",(long)self.viewrating.tag);
    
    
    [self.contentView addSubview:self.viewrating];
}
-(void)floatRatingView:(FloatRatingView *)ratingView didUpdate:(float)rating
{
    
    rating  = ratingView.rating;
    NSNumber *ratingNumber = [NSNumber numberWithFloat:rating];
    [[NSUserDefaults standardUserDefaults] setValue:ratingNumber forKey:@"rating"];
    [[NSUserDefaults standardUserDefaults] setValue:self.field.key forKey:@"key"];
    self.field.value = ratingNumber;
    NSLog(@"%@",self.field.key);
    [self update];
    if(self.field.action) self.field.action(self);
...
- (void)update
{
    ...
    //self.detailTextLabel.text = [self.field fieldDescription];
    NSLog(@"%@",self.field.key);
    NSLog(@"%@",self.field.value);
    if (self.field.value != nil)
    {
        self.viewrating.rating = [self.field.value floatValue];
    }
...
@interface FXFormOptionPickerCell () <UIPickerViewDataSource, UIPickerViewDelegate,UIGestureRecognizerDelegate>
{
    UIView *doneView;
}
...
//        {
            ...
            doneView=[[UIView alloc]initWithFrame:CGRectMake(0, self.window.bounds.size.height - self.pickerView.frame.size.height-30, self.window.bounds.size.width, 50)];
            [doneView setBackgroundColor:[UIColor lightGrayColor]];
